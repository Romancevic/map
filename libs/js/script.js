...
function setCountryInfo(result) {
    ...
    fetch(`https://eventregistry.org/api/v1/article/getArticles?resultType=articles&keyword=${result['data'][0]['countryName']}&articlesCount=1&apiKey=f27e6895-8f65-4df1-a6bc-8caef60d73e8`)
        .then(data => data.json())
        .then(data => {
    ...
    fetch(`https://api.covid19api.com/dayone/country/${result['data'][0]['countryCode']}`)
        .then(data => data.json())
        .then(data => {
    ...
    fetch('../../iso.json')
        .then(iso => iso.json())
        .then(iso => {
            fetch(`https://restcountries.eu/rest/v2/alpha/${encodeURI(result['data'][0]['countryCode'])}`)
                .then(data => data.json())
                .then(data => {
                    ...
                    fetch(`https://api.exchangeratesapi.io/latest?base=USD&symbols=${encodeURI(data['currencies'][0]['code'])}`)
                        .then(info => info.json())
                        .then(info => {
                            let put = [
                                {
    ...
    fetch(`https://cors-anywhere.herokuapp.com/http://api.weatherstack.com/forecast?access_key=e8e0eec9f566deb7dd6062bead55dc02&query=${encodeURI(result['data'][0]['capital'])}`)
        .then(data => data.json())
        .then(info => {
